# -*- coding: utf-8 -*-
"""
🚀 PROCESADOR INTEGRAL WINDOWS v2.0 - SIN TRABAS
===============================================
Procesador robusto compatible con Windows que maneja TODA la ingesta cognitiva sin trabarse.

CARACTERÍSTICAS ANTI-TRABAS WINDOWS:
✅ Manejo de errores individual por documento
✅ Timeouts usando threading (compatible Windows)
...
class ProcesadorIntegralWindows:
    """
    🚀 Procesador robusto compatible con Windows que no se traba
    
    ANTI-TRABAS WINDOWS:
    - Procesamiento individual con timeouts usando threading
    ...
    def __init__(self):
        self.version = "ProcesadorIntegralWindows_v2.0"
    ...
    def _crear_tabla_perfiles_autorales(self, cursor):
        ...
        CREATE TABLE IF NOT EXISTS perfiles_autorales (
            ...
            -- METADATOS
            perfil_json TEXT,
            procesado_con TEXT DEFAULT 'ProcesadorIntegralWindows_v2.0'
        )
        ''')
    
    def procesar_todo_robusto(self) -> Dict:
        """
        🚀 PROCESAMIENTO COMPLETO ANTI-TRABAS WINDOWS
        
        CARACTERÍSTICAS:
        - Timeout por documento usando threading
        - Manejo individual de errores
        - Progress tracking
        - Limpieza de memoria
        """
        
        print("🚀 INICIANDO PROCESAMIENTO INTEGRAL WINDOWS v2.0")
        print("=" * 60)
    ...
    def _procesar_documento_individual(self, doc_path: Path, numero: int, total: int):
        """
        Procesa un documento individual con protección anti-trabas usando threading
        
        PROTECCIONES WINDOWS:
        - Timeout por documento usando threading
        - Reintentos automáticos  
        ...
        for intento in range(1, MAX_REINTENTOS + 1):
            try:
                # Procesar documento con timeout usando threading
                resultado = self._procesar_con_timeout(doc_path, MAX_TIEMPO_POR_DOCUMENTO)
                ...
                if resultado and resultado.get("success"):
                    ...
                    error_msg = resultado.get("error", "Error desconocido") if resultado else "Timeout"
                    print(f"⚠️ Intento {intento} falló: {error_msg}")
    ...
    def _procesar_con_timeout(self, doc_path: Path, timeout_seconds: int) -> Optional[Dict]:
        """Procesa documento con timeout usando threading (compatible Windows)"""
        
        resultado = {"success": False, "error": "No iniciado"}
        
        def target():
            try:
                resultado.update(self._procesar_documento_contenido(doc_path))
            except Exception as e:
                resultado.update({"success": False, "error": str(e)})
        ...
        if thread.is_alive():
            ...
            print(f"⏱️ TIMEOUT después de {timeout_seconds}s")
            return {"success": False, "error": f"Timeout después de {timeout_seconds}s"}
    ...
    def _extraer_texto_pdf(self, pdf_path: Path) -> str:
        ...
        try:
                ...
                for pagina_num in range(total_paginas):
                    try:
                        ...
                        if (pagina_num + 1) % 10 == 0:
                            print(f"  📖 Procesadas {pagina_num + 1}/{total_paginas} páginas")
                            
                    except Exception as e:
                        print(f"⚠️ Error en página {pagina_num + 1}: {e}")
                        continue
            ...
            print(f"✅ Texto limpio: {len(texto_final)} caracteres")
    ...
    def _guardar_perfil_en_db(self, doc_path: Path, perfil: Dict) -> Dict:
        ...
        try:
            ...
            with sqlite3.connect(db_path) as conn:
                ...
                conn.commit()
                print(f"💾 Perfil guardado para: {autor_detectado}")
    ...
    def _generar_reporte_final(self, tiempo_total: float) -> Dict:
        ...
        total_docs = self.documentos_procesados + self.documentos_fallidos
        ...
        reporte = {
            ...
            "tasa_exito": f"{(self.documentos_procesados / total_docs * 100):.1f}%" if total_docs > 0 else "0%",
            "errores": self.errores_procesamiento,
            "version_procesador": self.version
        }
        ...
        if self.documentos_procesados > 0:
            print("\n🚀 PRÓXIMOS PASOS:")
            print("1. Verificar análisis: python colaborative/scripts/comparador_mentes.py")
            print("2. Iniciar sistema web: .\\iniciar_sistema.bat")
            print("3. Explorar autores: http://127.0.0.1:5002/referencias")
        
        return reporte

def main():
    """Función principal del procesador mejorado Windows"""
    
    print("🚀 PROCESADOR INTEGRAL WINDOWS v2.0")
    print("Procesador robusto anti-trabas compatible con Windows")
    print("=" * 60)
    
    try:
        procesador = ProcesadorIntegralWindows()
        resultado = procesador.procesar_todo_robusto()
        
        # Guardar reporte
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        reporte_path = BASE_DIR / f"reporte_procesamiento_windows_{timestamp}.json"
        ...
        if resultado["status"] == "completed" and resultado["documentos_procesados"] > 0:
            ...
            print("El sistema está listo para usar con los nuevos análisis cognitivos")
        elif resultado["status"] == "success" and resultado["documentos_procesados"] == 0:
            print("\nℹ️ No había documentos nuevos para procesar")
            print("El sistema sigue funcionando con los análisis existentes")
